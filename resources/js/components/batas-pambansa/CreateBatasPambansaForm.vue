...
const validateForm = () => {
  ...
  if (!formData.bp_number.trim()) {
    errors.bp_number = 'B.P. Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/batas-pambansa', formData, {
      headers: {
    ...
    if (response.data.success) {
      toast.success('Batas Pambansa record created successfully!');
      clearForm();
    } else {
    ...
    console.error('Error creating batas pambansa:', error);
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create batas pambansa record';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="bp_number">
            B.P. Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="bp_number"
            v-model="formData.bp_number"
            placeholder="e.g., B.P. Blg. 123"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.bp_number }"
          />
          <p v-if="errors.bp_number" class="text-sm text-red-500">{{ errors.bp_number }}</p>
        </div>
