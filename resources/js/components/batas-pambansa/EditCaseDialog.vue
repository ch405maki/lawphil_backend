...
const updateCase = async () => {
    if (!props.raData || !props.raData.id) {
        toast.error('Error: No batas pambansa data selected.');
        return;
    }
    errors.value = {};
    ...
    try {
        ...
        if (response.data.success) {
            toast.success('Batas Pambansa updated successfully!');
            emit('update:open', false);
            emit('saved');
        } else {
            throw new Error(response.data.message || 'Failed to update batas pambansa');
        }
    } catch (error: any) {
        console.error('Error updating batas pambansa:', error);

        if (error.response?.data?.errors) {
            errors.value = error.response.data.errors;
            toast.error('Please check the form for errors');
        } else {
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update batas pambansa';
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            ...
            <div class="space-y-4 py-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="bp_number">B.P. Number</Label>
                        <Input
                            id="bp_number"
                            v-model="formData.bp_number"
                            :class="{ 'border-destructive': errors.bp_number }"
                            :disabled="processing"
                        />
                        <p v-if="errors.bp_number" class="text-xs text-destructive">{{ errors.bp_number[0] }}</p>
                    </div>
                    <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <Textarea
                        ...
                        rows="3"
                        placeholder="Enter the description of the batas pambansa"
                        :disabled="processing"
                    />
