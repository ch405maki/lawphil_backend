...
const updateCase = async () => {
    if (!props.mcData || !props.mcData.id) {
        toast.error('Error: No memorandum circular data selected.');
        return;
    }
    errors.value = {};
    ...
    try {
        const response = await axios.post(
            `/api/v1/mc/${editingId.value ?? props.mcData.id}`,
            {
        ...
        if (response.data.success) {
            toast.success('Memorandum Circular updated successfully!');
            emit('update:open', false);
            emit('saved');
        } else {
            throw new Error(response.data.message || 'Failed to update memorandum circular');
        }
    } catch (error: any) {
        console.error('Error updating memorandum circular:', error);

        if (error.response?.data?.errors) {
            errors.value = error.response.data.errors;
            toast.error('Please check the form for errors');
        } else {
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update memorandum circular';
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <div class="flex items-start justify-between gap-2 pr-6">
                    <DialogTitle>
                        {{ isDuplicateEdit ? 'Editing Duplicated Record' : 'Edit Memorandum Circular Information' }}
                    </DialogTitle>
                ...
                <DialogDescription>
                    <template v-if="isDuplicateEdit">
                        ...
                        <span v-if="formData.mc_number" class="font-medium text-foreground">(M.C. No. {{ formData.mc_number }})</span>. Saving changes
                        updates this new record — the original is left untouched.
                    </template>
                    <template v-else> Update the memorandum circular details below. </template>
                </DialogDescription>
            </DialogHeader>
            ...
            <div class="space-y-4 py-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="mc_number">Memorandum Circular Number</Label>
                        <Input
                            id="mc_number"
                ...
                <div class="space-y-2">
                    ...
                    <Textarea
                        ...
                        rows="3"
                        placeholder="Enter the description of the Memorandum Circular"
                        :disabled="processing"
                    />
