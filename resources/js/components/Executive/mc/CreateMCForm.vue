...
const validateForm = () => {
  ...
  // Validate Memorandum Circular Number
  if (!formData.mc_number.trim()) {
    errors.mc_number = 'Memorandum Circular Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/mc', formData, {
      headers: {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.message || 'Failed to create memorandum circular');
    }
    
  } catch (error: any) {
    console.error('Error creating memorandum circular:', error);
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create memorandum circular';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="mc_number">
            Memorandum Circular Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="mc_number"
            v-model="formData.mc_number"
            placeholder="e.g., MC No. 123456"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.mc_number }"
          />
          <p v-if="errors.mc_number" class="text-sm text-red-500">{{ errors.mc_number }}</p>
        </div>
