...
// Custom Components
import EditCaseDialog from '@/components/Executive/mc/EditCaseDialog.vue';
import DeleteCaseDialog from '@/components/Executive/mc/DeleteCaseDialog.vue';
...
// Helper function to extract year from filename (e.g., "mc_1211_2026.html" -> "2026")
...
const generatePdfUrl = (url: string) => {
  ...
  // Kung relative path lang (hal. "mc_1211_2026.html")
  const year = getYearFromFilename(url);
  const yearFolder = year ? `mc${year}/` : '';
  const pdfFileName = url.replace('.html', '.pdf');
  
  // Walang double slash, at dynamic ang taon!
  return `https://lawphil.net/executive/mc/${yearFolder}pdf/${pdfFileName}`;
};
...
const generateHtmlUrl = (url: string) => {
  ...
  return `https://lawphil.net/executive/mc/${yearFolder}${url}`;
};

// Computed properties for multiple deletion
const isAllSelectedOnPage = computed(() => {
  return mc.value.data?.length > 0 && selectedIds.value.length === mc.value.data?.length;
});

const isSomeSelected = computed(() => {
  return selectedIds.value.length > 0 && selectedIds.value.length < mc.value.data?.length;
});
...
const pageNumbers = computed(() => {
  const total = mc.value.last_page;
  const current = mc.value.current_page;
  const delta = 2;
  const range = [];
...
const fetchData = async (page = 1, signal?: AbortSignal) => {
  ...
  try {
    const response = await axios.get('/api/v1/mc', {
      params: {
        search: search.value,
        page: page,
    ...
    if (error.name !== 'AbortError' && error.code !== 'ERR_CANCELED') {
      ...
      toast.error('Failed to fetch memorandum order data');
    }
  } finally {
    if (!signal) {
      loading.value = false;
...
watch(() => props.refreshTrigger, (newVal, oldVal) => {
  if (newVal !== oldVal) {
    fetchData(mc.value.current_page);
  }
});

const openEdit = (item: any) => {
  currentMC.value = { ...item };
...
const toggleSelectAllOnPage = () => {
  if (isAllSelectedOnPage.value) {
    selectedIds.value = selectedIds.value.filter(id => 
      !mc.value.data.some((item: any) => item.id === id)
    );
  } else {
    const pageIds = mc.value.data.map((item: any) => item.id);
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (selectAllAcrossPages.value) {
      response = await axios.post('/api/mc/bulk-delete', {
        select_all: true,
        filters: {
          search: search.value,
          year: filterState.year,
          ...
          <div class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[200px] max-w-[320px]">
              ...
              <Input 
                v-model="search" 
                @input="handleSearch"
                placeholder="Search Memorandum Circular No. or Description..." 
                class="pl-9"
              />
            </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead class="w-12">
                <div class="flex flex-col gap-1">
                  <Checkbox 
                    ...
                    :disabled="loading || mc.data?.length === 0"
                  />
              ...
              <TableHead class="w-[15%]">Memorandum Circular No. & Date</TableHead>
          ...
          <TableBody>
            <template v-if="loading && mc.data?.length === 0">
            ...
            <TableRow v-for="item in mc.data" :key="item.id" class="group">
              <TableCell>
                <Checkbox 
              ...
              <TableCell></TableCell>
              <TableCell>
                <div class="font-semibold">{{ item.mc_number }}</div>
            ...
            <TableRow v-if="mc.data?.length === 0 && !loading">
              <TableCell colspan="7" class="text-center py-8">
                <div class="flex flex-col items-center gap-2">
                  <AlertCircle class="h-8 w-8 text-muted-foreground" />
                  <p class="text-muted-foreground">No memorandum circular found</p>
                </div>
              </TableCell>
            </TableRow>
          </TableBody>
        ...
        <div class="border-t px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-4">
            <p class="text-sm text-muted-foreground">
              Showing {{ mc.from }} to {{ mc.to }} of {{ mc.total }} entries
            </p>
            <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            <Button 
              variant="outline" 
              size="sm"
              @click="fetchData(mc.current_page - 1)" 
              :disabled="mc.current_page === 1 || loading"
            >
            ...
            <template v-for="(page, index) in pageNumbers" :key="index">
              <Button 
                ...
                :class="mc.current_page === page ? 'bg-primary text-primary-foreground' : ''"
                :disabled="loading"
              >
                {{ page }}
              </Button>
            ...
            <Button 
              ...
              size="sm"
              @click="fetchData(mc.current_page + 1)" 
              :disabled="mc.current_page === mc.last_page || loading"
            >
