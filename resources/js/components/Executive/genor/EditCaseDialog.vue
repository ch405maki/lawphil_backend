...
// Watch for genorData changes to populate form
watch(
    () => props.genorData,
    (newData) => {
        if (newData && newData.id) {
            formData.value = {
                genor_number: newData.genor_number || '',
...
const updateCase = async () => {
    if (!props.genorData || !props.genorData.id) {
        toast.error('Error: No general order data selected.');
        return;
    }
    errors.value = {};
    ...
    try {
        const response = await axios.post(
            `/api/v1/genor/${editingId.value ?? props.genorData.id}`,
            {
                genor_number: formData.value.genor_number,
        ...
        if (response.data.success) {
            ...
            throw new Error(response.data.message || 'Failed to update general order');
        }
    } catch (error: any) {
        ...
        if (error.response?.data?.errors) {
            ...
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update general order';
...
const duplicateAsNew = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            formData.value = {
                genor_number: created.genor_number || '',
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <div class="flex items-start justify-between gap-2 pr-6">
                    <DialogTitle>
                        {{ isDuplicateEdit ? 'Editing Duplicated Record' : 'Edit General Order Information' }}
                    </DialogTitle>
                ...
                <DialogDescription>
                    <template v-if="isDuplicateEdit">
                        ...
                        <span v-if="formData.genor_number" class="font-medium text-foreground">(G.O. No. {{ formData.genor_number }})</span>. Saving
                        changes updates this new record — the original is left untouched.
                    </template>
                    <template v-else> Update the general order details below. </template>
                </DialogDescription>
            </DialogHeader>
            ...
            <div class="space-y-4 py-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="genor_number">General Order Number</Label>
                        <Input
                            id="genor_number"
                            v-model="formData.genor_number"
                            :class="{ 'border-destructive': errors.genor_number }"
                            :disabled="processing"
                        />
                        <p v-if="errors.genor_number" class="text-xs text-destructive">{{ errors.genor_number[0] }}</p>
                    </div>
                    <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <Textarea
                        ...
                        rows="3"
                        placeholder="Enter the description of the general order"
                        :disabled="processing"
                    />
