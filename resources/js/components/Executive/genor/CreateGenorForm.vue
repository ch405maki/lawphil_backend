...
const validateForm = () => {
  ...
  // Validate General Order Number
  if (!formData.genor_number.trim()) {
    errors.genor_number = 'General Order Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/genor', formData, {
      headers: {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.message || 'Failed to create general order');
    }
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create general order record';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="genor_number">
            General Order Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="genor_number"
            v-model="formData.genor_number"
            placeholder="e.g., General Order No. 123456"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.genor_number }"
          />
          <p v-if="errors.genor_number" class="text-sm text-red-500">{{ errors.genor_number }}</p>
        </div>
