...
// Custom Components
import EditCaseDialog from '@/components/Executive/execord/EditCaseDialog.vue';
import DeleteCaseDialog from '@/components/Executive/execord/DeleteCaseDialog.vue';
...
// Helper function to extract year from filename (e.g., "execord_1211_2026.html" -> "2026")
...
const generatePdfUrl = (url: string) => {
  ...
  const yearFolder = year ? `eo${year}/` : '';
  const pdfFileName = url.replace('.html', '.pdf');
  
  // Walang double slash, at dynamic ang taon!
  return `https://lawphil.net/executive/execord/${yearFolder}pdf/${pdfFileName}`;
};
...
const generateHtmlUrl = (url: string) => {
  ...
  // Kung relative path lang (hal. "execord_1211_2026.html")
  const year = getYearFromFilename(url);
  const yearFolder = year ? `eo${year}/` : '';
  
  return `https://lawphil.net/executive/execord/${yearFolder}${url}`;
};

// Computed properties for multiple deletion
const isAllSelectedOnPage = computed(() => {
  return execord.value.data?.length > 0 && selectedIds.value.length === execord.value.data?.length;
});

const isSomeSelected = computed(() => {
  return selectedIds.value.length > 0 && selectedIds.value.length < execord.value.data?.length;
});
...
const pageNumbers = computed(() => {
  const total = execord.value.last_page;
  const current = execord.value.current_page;
  const delta = 2;
  const range = [];
...
const fetchData = async (page = 1, signal?: AbortSignal) => {
  ...
  try {
    const response = await axios.get('/api/v1/execord', {
      params: {
        search: search.value,
        page: page,
...
watch(() => props.refreshTrigger, (newVal, oldVal) => {
  if (newVal !== oldVal) {
    fetchData(execord.value.current_page);
  }
});

const openEdit = (item: any) => {
  currentExecord.value = { ...item };
...
const toggleSelectAllOnPage = () => {
  if (isAllSelectedOnPage.value) {
    selectedIds.value = selectedIds.value.filter(id => 
      !execord.value.data.some((item: any) => item.id === id)
    );
  } else {
    const pageIds = execord.value.data.map((item: any) => item.id);
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (selectAllAcrossPages.value) {
      response = await axios.post('/api/execord/bulk-delete', {
        select_all: true,
        filters: {
          search: search.value,
          year: filterState.year,
          ...
          <div class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[200px] max-w-[320px]">
              ...
              <Input 
                v-model="search" 
                @input="handleSearch"
                placeholder="Search Executive Order No. or Description..." 
                class="pl-9"
              />
            </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead class="w-12">
                <div class="flex flex-col gap-1">
                  <Checkbox 
                    ...
                    :disabled="loading || execord.data?.length === 0"
                  />
              ...
              <TableHead class="w-[15%]">Executive Order No. & Date</TableHead>
          ...
          <TableBody>
            <template v-if="loading && execord.data?.length === 0">
            ...
            <TableRow v-for="item in execord.data" :key="item.id" class="group">
              <TableCell>
                <Checkbox 
              ...
              <TableCell></TableCell>
              <TableCell>
                <div class="font-semibold">{{ item.execord_number }}</div>
            ...
            <TableRow v-if="execord.data?.length === 0 && !loading">
              <TableCell colspan="7" class="text-center py-8">
                <div class="flex flex-col items-center gap-2">
                  <AlertCircle class="h-8 w-8 text-muted-foreground" />
                  <p class="text-muted-foreground">No executive order found</p>
                </div>
              </TableCell>
            </TableRow>
          </TableBody>
        ...
        <div class="border-t px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-4">
            <p class="text-sm text-muted-foreground">
              Showing {{ execord.from }} to {{ execord.to }} of {{ execord.total }} entries
            </p>
            <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            <Button 
              variant="outline" 
              size="sm"
              @click="fetchData(execord.current_page - 1)" 
              :disabled="execord.current_page === 1 || loading"
            >
            ...
            <template v-for="(page, index) in pageNumbers" :key="index">
              <Button 
                ...
                :class="execord.current_page === page ? 'bg-primary text-primary-foreground' : ''"
                :disabled="loading"
              >
                {{ page }}
              </Button>
            ...
            <Button 
              ...
              size="sm"
              @click="fetchData(execord.current_page + 1)" 
              :disabled="execord.current_page === execord.last_page || loading"
            >
