...
const updateCase = async () => {
    ...
    try {
        const response = await axios.post(
            `/api/v1/execord/${props.execordData.id}`,
            {
...
<template>
    <Dialog :open="open" @update:open="closeDialog">
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <DialogTitle>Edit Executive Order Information</DialogTitle>
                <DialogDescription> Update the executive order details below. </DialogDescription>
            </DialogHeader>
