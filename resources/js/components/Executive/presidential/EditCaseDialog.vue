...
watch(() => props.presdecData, (newData) => {
...
const updateCase = async () => {
  ...
  try {
    const response = await axios.post(
      `/api/v1/presidential/${props.presdecData.id}`,
      {
...
<template>
  <Dialog :open="open" @update:open="closeDialog">
    <DialogContent class="max-w-2xl max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle>Edit Presidential Decrees Information</DialogTitle>
        <DialogDescription>
          Update the presidential decrees details below.
        </DialogDescription>
      </DialogHeader>
