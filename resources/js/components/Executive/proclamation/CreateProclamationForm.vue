...
const validateForm = () => {
  ...
  // Validate Proclamation Number
  if (!formData.proc_number.trim()) {
    errors.proc_number = 'Proclamation Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/proclamations', formData, {
      headers: {
    ...
    if (response.data.success) {
      toast.success('Proclamation record created successfully!');
      clearForm();
    } else {
    ...
    console.error('Error creating proclamation:', error);
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create proclamation record';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="proc_number">
            Proclamation Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="proc_number"
            v-model="formData.proc_number"
            placeholder="e.g., Proclamation No. 123456"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.proc_number }"
          />
          <p v-if="errors.proc_number" class="text-sm text-red-500">{{ errors.proc_number }}</p>
        </div>
