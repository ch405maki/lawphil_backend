...
const isAllSelectedOnPage = computed(() => {
  return proc.value.data?.length > 0 && selectedIds.value.length === proc.value.data?.length;
});

const isSomeSelected = computed(() => {
  return selectedIds.value.length > 0 && selectedIds.value.length < proc.value.data?.length;
});
...
const toggleSelectAllOnPage = () => {
  if (isAllSelectedOnPage.value) {
    selectedIds.value = selectedIds.value.filter(id => 
      !proc.value.data.some((item: any) => item.id === id)
    );
  } else {
    const pageIds = proc.value.data.map((item: any) => item.id);
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (selectAllAcrossPages.value) {
      response = await axios.post('/api/proclamations/bulk-delete', {
        select_all: true,
        filters: {
          search: search.value,
          year: filterState.year,
              ...
              <TableHead class="w-12">
                <div class="flex flex-col gap-1">
                  <Checkbox 
                    ...
                    :disabled="loading || proc.data?.length === 0"
                  />
