...
const updateCase = async () => {
    ...
    try {
        const response = await axios.post(
            `/api/v1/proclamations/${editingId.value ?? props.procData.id}`,
            {
...
const duplicateAsNew = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            formData.value = {
                proc_number: created.proc_number || '',
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <div class="flex items-start justify-between gap-2 pr-6">
                    <DialogTitle>
                        {{ isDuplicateEdit ? 'Editing Duplicated Record' : 'Edit Proclamation Information' }}
                    </DialogTitle>
                ...
                <DialogDescription>
                    <template v-if="isDuplicateEdit">
                        ...
                        <span v-if="formData.proc_number" class="font-medium text-foreground">(Proclamation No. {{ formData.proc_number }})</span>.
                        Saving changes updates this new record — the original is left untouched.
                    </template>
                    <template v-else> Update the proclamation details below. </template>
                </DialogDescription>
            </DialogHeader>
