...
const validateForm = () => {
  ...
  // Validate Administrative Order Number
  if (!formData.ao_number.trim()) {
    errors.ao_number = 'Administrative Order Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/ao', formData, {
      headers: {
    ...
    if (response.data.success) {
      toast.success('Administrative Order record created successfully!');
      clearForm();
    } else {
    ...
    console.error('Error creating administrative order:', error);
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create administrative order record';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="ao_number">
            Administrative Order Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="ao_number"
            v-model="formData.ao_number"
            placeholder="e.g., Administrative Order No. 123456"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.ao_number }"
          />
          <p v-if="errors.ao_number" class="text-sm text-red-500">{{ errors.ao_number }}</p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-4">
        <div>
          ...
          <Textarea
            ...
            placeholder="Enter the description of the administrative order"
            :disabled="isLoading"
            rows="3"
