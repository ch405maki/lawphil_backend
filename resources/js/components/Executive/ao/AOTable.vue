...
// Custom Components
import EditCaseDialog from '@/components/Executive/ao/EditCaseDialog.vue';
import DeleteCaseDialog from '@/components/Executive/ao/DeleteCaseDialog.vue';
...
// Helper function to extract year from filename (e.g., "ao_1211_2026.html" -> "2026")
...
const generateHtmlUrl = (url: string) => {
  ...
  return `https://lawphil.net/executive/ao/${yearFolder}${url}`;
};

// Computed properties for multiple deletion
const isAllSelectedOnPage = computed(() => {
  return ao.value.data?.length > 0 && selectedIds.value.length === ao.value.data?.length;
});

const isSomeSelected = computed(() => {
  return selectedIds.value.length > 0 && selectedIds.value.length < ao.value.data?.length;
});
...
const pageNumbers = computed(() => {
  const total = ao.value.last_page;
  const current = ao.value.current_page;
  const delta = 2;
  const range = [];
...
const fetchData = async (page = 1, signal?: AbortSignal) => {
  ...
  try {
    const response = await axios.get('/api/v1/ao', {
      params: {
        search: search.value,
        page: page,
    ...
    if (error.name !== 'AbortError' && error.code !== 'ERR_CANCELED') {
      ...
      toast.error('Failed to fetch administrative order data');
    }
  } finally {
    if (!signal) {
      loading.value = false;
...
watch(() => props.refreshTrigger, (newVal, oldVal) => {
  if (newVal !== oldVal) {
    fetchData(ao.value.current_page);
  }
});

const openEdit = (item: any) => {
  currentAO.value = { ...item };
...
const toggleSelectAllOnPage = () => {
  if (isAllSelectedOnPage.value) {
    selectedIds.value = selectedIds.value.filter(id => 
      !ao.value.data.some((item: any) => item.id === id)
    );
  } else {
    const pageIds = ao.value.data.map((item: any) => item.id);
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (selectAllAcrossPages.value) {
      response = await axios.post('/api/ao/bulk-delete', {
        select_all: true,
        filters: {
          search: search.value,
          year: filterState.year,
          ...
          <div class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[200px] max-w-[320px]">
              ...
              <Input 
                v-model="search" 
                @input="handleSearch"
                placeholder="Search Administrative Order No. or Description..." 
                class="pl-9"
              />
            </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead class="w-12">
                <div class="flex flex-col gap-1">
                  <Checkbox 
                    ...
                    :disabled="loading || ao.data?.length === 0"
                  />
              ...
              <TableHead class="w-[15%]">Administrative Order No. & Date</TableHead>
          ...
          <TableBody>
            <template v-if="loading && ao.data?.length === 0">
            ...
            <TableRow v-for="item in ao.data" :key="item.id" class="group">
              <TableCell>
                <Checkbox 
            ...
            <TableRow v-if="ao.data?.length === 0 && !loading">
              <TableCell colspan="7" class="text-center py-8">
                <div class="flex flex-col items-center gap-2">
                  <AlertCircle class="h-8 w-8 text-muted-foreground" />
                  <p class="text-muted-foreground">No administrative order found</p>
                </div>
              </TableCell>
            </TableRow>
          </TableBody>
        ...
        <div class="border-t px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-4">
            <p class="text-sm text-muted-foreground">
              Showing {{ ao.from }} to {{ ao.to }} of {{ ao.total }} entries
            </p>
            <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            <Button 
              variant="outline" 
              size="sm"
              @click="fetchData(ao.current_page - 1)" 
              :disabled="ao.current_page === 1 || loading"
            >
            ...
            <template v-for="(page, index) in pageNumbers" :key="index">
              <Button 
                ...
                :class="ao.current_page === page ? 'bg-primary text-primary-foreground' : ''"
                :disabled="loading"
              >
                {{ page }}
              </Button>
            ...
            <Button 
              ...
              size="sm"
              @click="fetchData(ao.current_page + 1)" 
              :disabled="ao.current_page === ao.last_page || loading"
            >
