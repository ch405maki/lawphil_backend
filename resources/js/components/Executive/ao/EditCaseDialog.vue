...
// Watch for aoData changes to populate form
watch(
    () => props.aoData,
    (newData) => {
...
const updateCase = async () => {
    if (!props.aoData || !props.aoData.id) {
        toast.error('Error: No administrative order data selected.');
        return;
    }
    errors.value = {};
    ...
    try {
        ...
        if (response.data.success) {
            ...
            throw new Error(response.data.message || 'Failed to update administrative order');
        }
    } catch (error: any) {
        console.error('Error updating administrative order:', error);

        if (error.response?.data?.errors) {
            errors.value = error.response.data.errors;
            toast.error('Please check the form for errors');
        } else {
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update administrative order';
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            ...
            <div class="space-y-4 py-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="ao_number">Administrative Order Number</Label>
                        <Input
                            id="ao_number"
