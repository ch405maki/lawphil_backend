...
// Watch for moData changes to populate form
watch(
    () => props.moData,
    (newData) => {
        if (newData && newData.id) {
            formData.value = {
                mo_number: newData.mo_number || '',
...
const updateCase = async () => {
    if (!props.moData || !props.moData.id) {
        toast.error('Error: No memorandum order data selected.');
        return;
    }
    errors.value = {};
    ...
    try {
        const response = await axios.post(
            `/api/v1/mo/${editingId.value ?? props.moData.id}`,
            {
                mo_number: formData.value.mo_number,
        ...
        if (response.data.success) {
            ...
            throw new Error(response.data.message || 'Failed to update memorandum order');
        }
    } catch (error: any) {
        console.error('Error updating memorandum order:', error);

        if (error.response?.data?.errors) {
            errors.value = error.response.data.errors;
            toast.error('Please check the form for errors');
        } else {
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update memorandum order';
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <div class="flex items-start justify-between gap-2 pr-6">
                    <DialogTitle>
                        {{ isDuplicateEdit ? 'Editing Duplicated Record' : 'Edit Memorandum Order Information' }}
                    </DialogTitle>
                ...
                <DialogDescription>
                    <template v-if="isDuplicateEdit">
                        ...
                        <span v-if="formData.mo_number" class="font-medium text-foreground">(M.O. No. {{ formData.mo_number }})</span>. Saving changes
                        updates this new record — the original is left untouched.
                    </template>
                    <template v-else> Update the memorandum order details below. </template>
                </DialogDescription>
            </DialogHeader>
            ...
            <div class="space-y-4 py-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="mo_number">Memorandum Order Number</Label>
                        <Input
                            id="mo_number"
                            v-model="formData.mo_number"
                            :class="{ 'border-destructive': errors.mo_number }"
                            :disabled="processing"
                        />
                        <p v-if="errors.mo_number" class="text-xs text-destructive">{{ errors.mo_number[0] }}</p>
                    </div>
                    <div class="space-y-2">
