...
// Custom Components
import EditCaseDialog from '@/components/Executive/mo/EditCaseDialog.vue';
import DeleteCaseDialog from '@/components/Executive/mo/DeleteCaseDialog.vue';
import ExcelImportDialog from '@/components/Executive/mo/ExcelImportDialog.vue';
...
// Helper function to extract year from filename (e.g., "mo_1211_2026.html" -> "2026")
...
const generatePdfUrl = (url: string) => {
  ...
  // Kung relative path lang (hal. "mo_1211_2026.html")
  ...
  return `https://lawphil.net/executive/mo/${yearFolder}pdf/${pdfFileName}`;
};
...
const generateHtmlUrl = (url: string) => {
  ...
  return `https://lawphil.net/executive/mo/${yearFolder}${url}`;
};

// Computed properties for multiple deletion
const isAllSelectedOnPage = computed(() => {
  return mo.value.data?.length > 0 && selectedIds.value.length === mo.value.data?.length;
});

const isSomeSelected = computed(() => {
  return selectedIds.value.length > 0 && selectedIds.value.length < mo.value.data?.length;
});
...
const pageNumbers = computed(() => {
  const total = mo.value.last_page;
  const current = mo.value.current_page;
  const delta = 2;
  const range = [];
...
const fetchData = async (page = 1, signal?: AbortSignal) => {
  ...
  try {
    const response = await axios.get('/api/v1/mo', {
      params: {
        search: search.value,
        page: page,
...
const toggleSelectAllOnPage = () => {
  if (isAllSelectedOnPage.value) {
    selectedIds.value = selectedIds.value.filter(id => 
      !mo.value.data.some((item: any) => item.id === id)
    );
  } else {
    const pageIds = mo.value.data.map((item: any) => item.id);
...
const bulkDelete = async () => {
  ...
  try {
    ...
    if (selectAllAcrossPages.value) {
      response = await axios.post('/api/mo/bulk-delete', {
        select_all: true,
        filters: {
          search: search.value,
          year: filterState.year,
          ...
          <div class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[200px] max-w-[320px]">
              ...
              <Input 
                v-model="search" 
                @input="handleSearch"
                placeholder="Search Memorandum Order No. or Description..." 
                class="pl-9"
              />
            </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead class="w-12">
                <div class="flex flex-col gap-1">
                  <Checkbox 
                    ...
                    :disabled="loading || mo.data?.length === 0"
                  />
              ...
              <TableHead class="w-[15%]">Memorandum Order No. & Date</TableHead>
          ...
          <TableBody>
            <template v-if="loading && mo.data?.length === 0">
            ...
            <TableRow v-for="item in mo.data" :key="item.id" class="group">
              <TableCell>
                <Checkbox 
              ...
              <TableCell></TableCell>
              <TableCell>
                <div class="font-semibold">{{ item.mo_number }}</div>
            ...
            <TableRow v-if="mo.data?.length === 0 && !loading">
              <TableCell colspan="7" class="text-center py-8">
                <div class="flex flex-col items-center gap-2">
                  <AlertCircle class="h-8 w-8 text-muted-foreground" />
                  <p class="text-muted-foreground">No memorandum order found</p>
                </div>
              </TableCell>
            </TableRow>
          </TableBody>
        ...
        <div class="border-t px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-4">
            <p class="text-sm text-muted-foreground">
              Showing {{ mo.from }} to {{ mo.to }} of {{ mo.total }} entries
            </p>
            <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            <Button 
              variant="outline" 
              size="sm"
              @click="fetchData(mo.current_page - 1)" 
              :disabled="mo.current_page === 1 || loading"
            >
            ...
            <template v-for="(page, index) in pageNumbers" :key="index">
              <Button 
                ...
                :class="mo.current_page === page ? 'bg-primary text-primary-foreground' : ''"
                :disabled="loading"
              >
                {{ page }}
              </Button>
            ...
            <Button 
              ...
              size="sm"
              @click="fetchData(mo.current_page + 1)" 
              :disabled="mo.current_page === mo.last_page || loading"
            >
