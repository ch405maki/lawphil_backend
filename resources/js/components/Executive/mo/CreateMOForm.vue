...
const validateForm = () => {
  ...
  if (!formData.mo_number.trim()) {
    errors.mo_number = 'Memorandum Order Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/mo', formData, {
      headers: {
    ...
    if (response.data.success) {
      toast.success('Memorandum Order record created successfully!');
      clearForm();
    } else {
    ...
    console.error('Error creating memorandum order:', error);
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create memorandum order record';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="mo_number">
            Memorandum Order Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="mo_number"
            v-model="formData.mo_number"
            placeholder="e.g., Memorandum Order No. 123456"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.mo_number }"
          />
      ...
      <div class="grid grid-cols-1 gap-4">
        <div>
          ...
          <Textarea
            ...
            placeholder="Enter the description of the memorandum order"
            :disabled="isLoading"
            rows="3"
