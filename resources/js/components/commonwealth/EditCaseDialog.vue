...
const updateCase = async () => {
    if (!props.raData || !props.raData.id) {
        toast.error('Error: No commonwealth data selected.');
        return;
    }
    errors.value = {};
    ...
    try {
        const response = await axios.post(
            `/api/v1/commonwealth/${editingId.value ?? props.raData.id}`,
            {
                ca_number: formData.value.ca_number,
        ...
        if (response.data.success) {
            toast.success('Commonwealth updated successfully!');
            emit('update:open', false);
            emit('saved');
        } else {
            throw new Error(response.data.message || 'Failed to update commonwealth');
        }
    } catch (error: any) {
        ...
        if (error.response?.data?.errors) {
            ...
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update commonwealth';
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <div class="flex items-start justify-between gap-2 pr-6">
                    <DialogTitle>
                        {{ isDuplicateEdit ? 'Editing Duplicated Record' : 'Edit Commonwealth Information' }}
                    </DialogTitle>
                ...
                <DialogDescription>
                    <template v-if="isDuplicateEdit">
                        ...
                        <span v-if="formData.ca_number" class="font-medium text-foreground">(C.A. No. {{ formData.ca_number }})</span>. Saving changes
                        updates this new record — the original is left untouched.
                    </template>
                    <template v-else> Update the commonwealth act details below. </template>
                </DialogDescription>
            </DialogHeader>
            ...
            <div class="space-y-4 py-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="ca_number">C.A. Number</Label>
                        <Input
                            id="ca_number"
                            v-model="formData.ca_number"
                            :class="{ 'border-destructive': errors.ca_number }"
                            :disabled="processing"
                        />
                        <p v-if="errors.ca_number" class="text-xs text-destructive">{{ errors.ca_number[0] }}</p>
                    </div>
                    <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <Textarea
                        ...
                        rows="3"
                        placeholder="Enter the description of the commonwealth act"
                        :disabled="processing"
                    />
