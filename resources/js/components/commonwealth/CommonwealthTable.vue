...
import EditCaseDialog from '@/components/commonwealth/EditCaseDialog.vue';
import DeleteCaseDialog from '@/components/commonwealth/DeleteCaseDialog.vue';
...
const generatePdfUrl = (pdfPath: string | null, url: string | null) => {
  if (pdfPath && pdfPath.trim() !== '') {
    ...
    const yearFolder = year ? `ca${year}/` : '';
    return `https://lawphil.net/statutes/comacts/${yearFolder}pdf/${pdfPath}`;
  }
  ...
  return `https://lawphil.net/statutes/comacts/${yearFolder}pdf/${pdfFileName}`;
};
...
const generateHtmlUrl = (url: string) => {
  ...
  const year = getYearFromFilename(url);
  const yearFolder = year ? `ca${year}/` : '';
  return `https://lawphil.net/statutes/comacts/${yearFolder}${url}`;
};
...
const fetchData = async (page = 1, signal?: AbortSignal) => {
  ...
  try {
    const response = await axios.get('/api/v1/commonwealth', {
      params: {
        search: search.value,
        page: page,
    ...
    if (error.name !== 'AbortError' && error.code !== 'ERR_CANCELED') {
      ...
      toast.error('Failed to fetch commonwealth data');
    }
  } finally {
    if (!signal) {
      loading.value = false;
    ...
    <Card>
      <CardContent class="p-0">
          ...
          <div class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[200px] max-w-[320px]">
              ...
              <Input
                v-model="search"
                @input="handleSearch"
                placeholder="Search C.A. No. or Description..."
                class="pl-9"
              />
            </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead class="w-[15%]">C.A. & Date</TableHead>
          ...
          <TableBody>
            ...
            <TableRow v-for="item in data.data" :key="item.id" class="group">
              <TableCell>
                <div class="font-semibold">{{ item.ca_number }}</div>
              ...
              <TableCell class="text-right">
                <div class="flex justify-end gap-2">
                  <Button v-if="can('commonwealth', 'update')" variant="ghost" size="icon" @click="openEdit(item)" :disabled="loading">
                    <SquarePen class="h-4 w-4" />
                  </Button>
                  <Button v-if="can('commonwealth', 'delete')" variant="ghost" size="icon" @click="confirmDelete(item.id)" :disabled="loading">
            ...
            <TableRow v-if="data.data?.length === 0 && !loading">
              <TableCell colspan="5" class="text-center py-8">
                <div class="flex flex-col items-center gap-2">
                  ...
                  <p class="text-muted-foreground">No commonwealth acts found</p>
                </div>
              </TableCell>
            </TableRow>
          </TableBody>
