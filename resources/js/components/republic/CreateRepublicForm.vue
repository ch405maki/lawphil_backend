...
const validateForm = () => {
  ...
  // Validate Republic Act Number
  if (!formData.ra_number.trim()) {
    errors.ra_number = 'Republic Act Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/republic', formData, {
      headers: {
    ...
    if (response.data.success) {
      toast.success('Republic Act record created successfully!');
      clearForm();
    } else {
    ...
    console.error('Error creating republic act:', error);
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create republic act record';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="ra_number">
            Republic Act Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="ra_number"
            v-model="formData.ra_number"
            placeholder="e.g., Republic Act No. 123456"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.ra_number }"
          />
          <p v-if="errors.ra_number" class="text-sm text-red-500">{{ errors.ra_number }}</p>
        </div>
