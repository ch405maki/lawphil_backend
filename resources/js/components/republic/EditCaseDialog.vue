...
const updateCase = async () => {
    ...
    try {
        const response = await axios.post(
            `/api/v1/republic/${editingId.value ?? props.raData.id}`,
            {
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <div class="flex items-start justify-between gap-2 pr-6">
                    <DialogTitle>
                        {{ isDuplicateEdit ? 'Editing Duplicated Record' : 'Edit Republic Act Information' }}
                    </DialogTitle>
                ...
                <DialogDescription>
                    <template v-if="isDuplicateEdit">
                        ...
                        <span v-if="formData.ra_number" class="font-medium text-foreground">(R.A. No. {{ formData.ra_number }})</span>. Saving changes
                        updates this new record — the original is left untouched.
                    </template>
                    <template v-else> Update the republic act details below. </template>
                </DialogDescription>
            </DialogHeader>
