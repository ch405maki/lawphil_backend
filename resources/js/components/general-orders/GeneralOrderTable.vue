...
// Hanapin mo ito sa taas at siguraduhin mong ganito ang itsura:
import EditGeneralOrderDialog from '@/components/general-orders/EditGeneralOrderDialog.vue';
...
// Helper function to generate HTML URL
...
const fetchData = async (page = 1, signal?: AbortSignal) => {
  ...
  try {
    const response = await axios.get('/api/general-orders', {
      params: {
        search: search.value,
        page: page,
    ...
    if (error.name !== 'AbortError' && error.code !== 'ERR_CANCELED') {
      ...
      toast.error('Failed to fetch general orders data');
    }
  } finally {
...
const bulkDelete = async () => {
  ...
  try {
    let response;
    const url = '/api/general-orders/bulk-delete';
...
<template>
  <div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            <div class="relative flex-1 min-w-[200px] max-w-[320px]">
              ...
              <Input 
                ...
                placeholder="Search G.O. No. or Title..." 
                class="pl-9"
              />
            </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead class="w-[15%]">G.O. & Date</TableHead>
          ...
          <TableBody>
            ...
            <TableRow v-for="item in cases.data" :key="item.id" class="group">
              ...
              <TableCell></TableCell>
              <TableCell>
                <div class="font-semibold">{{ item.go_number || 'N/A' }}</div>
    ...
    <EditGeneralOrderDialog v-model:open="showEditModal" :order-data="currentCase" @saved="refreshData" />
