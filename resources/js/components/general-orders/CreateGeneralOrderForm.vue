...
const validateForm = () => {
    ...
    if (!formData.go_number.trim()) {
        errors.go_number = 'G.O. Number is required';
        isValid = false;
    }
    if (!formData.date) {
...
const submitForm = async () => {
    ...
    try {
        const response = await axios.post('/api/general-orders', formData, {
            headers: {
        ...
        if (response.data.success) {
            toast.success('General Order created successfully!');
            clearForm();
        } else {
    ...
    <div>
        <form @submit.prevent="submitForm" class="space-y-4">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div>
                    <Label for="go_number">G.O. Number <span class="text-red-500">*</span></Label>
                    <Input
                        id="go_number"
                        v-model="formData.go_number"
                        placeholder="e.g., G.O. No. 1"
                        :disabled="isLoading"
                        :class="{ 'border-red-500': errors.go_number }"
                    />
                    <p v-if="errors.go_number" class="text-sm text-red-500">{{ errors.go_number }}</p>
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <Input
                        ...
                        placeholder="e.g., Pres. Ferdinand Marcos Jr."
                        :disabled="isLoading"
                        :class="{ 'border-red-500': errors.signatory }"
                    />
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <Textarea
                        ...
                        placeholder="Enter the order title or citation"
                        :disabled="isLoading"
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <Input
                            ...
                            type="text"
                            placeholder="Enter PDF path or URL (e.g., /uploads/pdfs/order.pdf)"
                            :disabled="isLoading || !formData.pdf_availability"
                            :class="{ 'border-red-500': errors.pdf_path }"
                            class="w-full"
                        />
                        <p v-if="errors.pdf_path" class="mt-1 text-sm text-red-500">{{ errors.pdf_path }}</p>
                        <p v-if="formData.pdf_availability" class="mt-1 text-xs text-muted-foreground">
            ...
            <div class="mt-6 flex flex-col justify-between gap-4 border-t pt-6 md:flex-row">
                ...
                <div class="order-1 flex justify-end gap-2 md:order-2">
