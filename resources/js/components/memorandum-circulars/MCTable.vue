...
const emit = defineEmits<{
  (e: 'refresh'): void;
  (e: 'edit', item: any): void; // Para ipasa sa Parent ang pag-edit
}>();
...
// Helper functions para sa URL (MC specific)
const generateHtmlUrl = (url: string | null | undefined): string => {
  if (!url) return '#';
  if (url.startsWith('http')) return url;
  return `https://lawphil.net/executive/memocirc/${url}`;
};
...
const isAllSelectedOnPage = computed(() => {
  return circulars.value.data?.length > 0 && selectedIds.value.length === circulars.value.data?.length;
});

const isSomeSelected = computed(() => {
  return selectedIds.value.length > 0 && selectedIds.value.length < circulars.value.data?.length;
});
...
const pageNumbers = computed(() => {
    const total = circulars.value.last_page;
    const current = circulars.value.current_page;
    const delta = 2;
    const range = [];
...
// Fetch Data (Tumuturo sa Memorandum Circulars API)
const fetchData = async (page = 1, signal?: AbortSignal) => {
  if (!signal) loading.value = true;
  
  try {
    const response = await axios.get('/api/memorandum-circulars', {
      params: {
        search: search.value,
        page: page,
    ...
    if (error.name !== 'AbortError' && error.code !== 'ERR_CANCELED') {
      ...
      toast.error('Failed to fetch memorandum circulars data');
    }
  } finally {
...
watch(() => props.refreshTrigger, () => {
  fetchData(circulars.value.current_page);
});
...
const toggleSelectAllOnPage = () => {
  if (isAllSelectedOnPage.value) {
    selectedIds.value = selectedIds.value.filter(id => 
      !circulars.value.data.some((item: any) => item.id === id)
    );
  } else {
    const pageIds = circulars.value.data.map((item: any) => item.id);
...
const bulkDelete = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/memorandum-circulars/bulk-delete', payload);
...
// Single Delete Logic (Nasa loob na ng file para malinis)
const deleteRecord = async () => {
    isDeleting.value = true;
    try {
        const response = await axios.delete(`/api/memorandum-circulars/${deleteId.value}`);
        if (response.data.success) {
  ...
  <div>
    <Card class="border-none shadow-none bg-transparent">
      <CardContent class="p-0">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
          ...
          <div class="flex flex-wrap items-center gap-3">
            <div class="relative min-w-[300px]">
              <Search v-if="!searchLoading" class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Loader2 v-else class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 animate-spin text-primary" />
              <Input 
                v-model="search" 
                @input="handleSearch"
                placeholder="Search M.C. Number or Subject..." 
                class="pl-9"
              />
            </div>
            ...
            <Button variant="outline" @click="resetFilters" size="icon" :disabled="searchLoading">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead class="w-12 px-4">
                  <div class="flex flex-col gap-1">
                    <Checkbox 
                      ...
                      :disabled="loading || circulars.data?.length === 0"
                    />
                ...
                <TableHead class="w-[20%]">M.C. Number & Date</TableHead>
                <TableHead class="w-[35%]">Subject / Description</TableHead>
                <TableHead class="w-[10%]">Reference</TableHead>
            ...
            <TableBody>
              <template v-if="loading && circulars.data?.length === 0">
                <TableRow v-for="i in 5" :key="i">
                  <TableCell colspan="8"><Skeleton class="h-12 w-full" /></TableCell>
                </TableRow>
              </template>
              
              <TableRow v-else v-for="item in circulars.data" :key="item.id" class="group">
                ...
                <TableCell></TableCell>
                <TableCell>
                  <div class="font-bold text-primary">{{ item.mc_number }}</div>
                  <div class="text-xs text-muted-foreground">{{ formatDate(item.date) }}</div>
                </TableCell>
                <TableCell>
                  <div class="text-sm font-medium line-clamp-2 group-hover:text-primary transition-colors">
                    {{ item.subject || 'No subject provided' }}
                  </div>
                </TableCell>
                <TableCell>
                  <div class="text-xs font-medium">{{ item.reference || '—' }}</div>
                </TableCell>
                <TableCell class="text-center">
                  <a v-if="item.url" :href="generateHtmlUrl(item.url)" target="_blank" class="text-primary hover:underline inline-flex items-center gap-1">
              ...
              <TableRow v-if="circulars.data?.length === 0 && !loading">
                <TableCell colspan="8" class="text-center py-8">
                  <div class="flex flex-col items-center gap-2">
                    <AlertCircle class="h-8 w-8 text-muted-foreground" />
                    <p class="text-muted-foreground">No Memorandum Circulars found.</p>
                  </div>
                </TableCell>
              </TableRow>
            </TableBody>
        ...
        <div class="flex items-center justify-between px-2 py-4 mt-2">
          <div class="flex items-center gap-4">
            <p class="text-sm text-muted-foreground">Showing {{ circulars.from || 0 }} to {{ circulars.to || 0 }} of {{ circulars.total }}</p>
            <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            <Button variant="outline" size="sm" @click="fetchData(circulars.current_page - 1)" :disabled="circulars.current_page === 1 || loading">
              <ChevronLeft class="h-4 w-4" />
            </Button>
            <template v-for="(page, index) in pageNumbers" :key="index">
              <Button v-if="page !== '...'" variant="outline" size="sm" @click="fetchData(page)" :class="circulars.current_page === page ? 'bg-primary text-primary-foreground' : ''" :disabled="loading">
                {{ page }}
              </Button>
              <span v-else class="px-2 text-muted-foreground">...</span>
            </template>
            <Button variant="outline" size="sm" @click="fetchData(circulars.current_page + 1)" :disabled="circulars.current_page === circulars.last_page || loading">
