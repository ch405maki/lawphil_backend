...
const validateForm = () => {
  ...
  if (!formData.act_number.trim()) {
    errors.act_number = 'Act Number is required';
    isValid = false;
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/api/v1/acts', formData, {
      headers: {
    ...
    if (response.data.success) {
      toast.success('Act record created successfully!');
      clearForm();
    } else {
    ...
    if (error.response?.data?.errors) {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to create act record';
  ...
  <div>
    <form @submit.prevent="submitForm" class="space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label for="act_number">
            Act Number <span class="text-red-500">*</span>
          </Label>
          <Input
            id="act_number"
            v-model="formData.act_number"
            placeholder="e.g., Act No. 123456"
            :disabled="isLoading"
            :class="{ 'border-red-500': errors.act_number }"
          />
          <p v-if="errors.act_number" class="text-sm text-red-500">{{ errors.act_number }}</p>
        </div>
