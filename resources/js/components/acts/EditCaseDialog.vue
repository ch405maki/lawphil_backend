...
const updateCase = async () => {
    if (!props.raData || !props.raData.id) {
        toast.error('Error: No act data selected.');
        return;
    }
    errors.value = {};
    ...
    try {
        const response = await axios.post(
            `/api/v1/acts/${editingId.value ?? props.raData.id}`,
            {
                act_number: formData.value.act_number,
        ...
        if (response.data.success) {
            ...
            throw new Error(response.data.message || 'Failed to update act');
        }
    } catch (error: any) {
        ...
        if (error.response?.data?.errors) {
            ...
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update act';
        ...
        <DialogContent class="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
                <div class="flex items-start justify-between gap-2 pr-6">
                    <DialogTitle>
                        {{ isDuplicateEdit ? 'Editing Duplicated Record' : 'Edit Act Information' }}
                    </DialogTitle>
                ...
                <DialogDescription>
                    <template v-if="isDuplicateEdit">
                        ...
                        <span v-if="formData.act_number" class="font-medium text-foreground">(Act No. {{ formData.act_number }})</span>. Saving
                        changes updates this new record — the original is left untouched.
                    </template>
                    <template v-else> Update the act details below. </template>
                </DialogDescription>
            </DialogHeader>
            ...
            <div class="space-y-4 py-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <Label for="act_number">Act Number</Label>
                        <Input
                            id="act_number"
                            v-model="formData.act_number"
                            :class="{ 'border-destructive': errors.act_number }"
                            :disabled="processing"
                        />
                        <p v-if="errors.act_number" class="text-xs text-destructive">{{ errors.act_number[0] }}</p>
                    </div>
                    <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <Textarea
                        ...
                        rows="3"
                        placeholder="Enter the description of the act"
                        :disabled="processing"
                    />
